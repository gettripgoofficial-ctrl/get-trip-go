...
function ItineraryAccordion({ itinerary, images, heroImage, themeColor }: {
          ...
          <div key={day.day} className="border border-gray-200 rounded-xl overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                <span className="text-xs font-bold border rounded-full px-3 py-1 bg-white" style={{ color: themeColor, borderColor: themeColor }}>
                  Day {day.day}
                </span>
...
export default function ThemeHolidayTabs({ pkg, themeColor, themeIcon }: { pkg: ThemePackage; themeColor: string; themeIcon: string }) {
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">
              A {pkg.duration} {pkg.theme.toLowerCase()} holiday to {pkg.destination}, {pkg.country} — curated around what makes a {pkg.theme.toLowerCase()} trip work.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {pkg.highlights.slice(0, 4).map((h) => (
                <div key={h} className="flex flex-col items-center text-center gap-1.5 rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow" style={{ backgroundColor: `${themeColor}12` }}>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <span className="text-white text-sm font-bold px-4 py-2 rounded-full inline-flex items-center gap-1.5" style={{ backgroundColor: themeColor }}>
              {themeIcon} {pkg.theme}
            </span>
          </div>
      ...
      {active === "Itinerary" && (
        <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6">
          ...
          {pkg.itinerary.length > 0 ? (
            <ItineraryAccordion itinerary={pkg.itinerary} images={pkg.images} heroImage={pkg.heroImage} themeColor={themeColor} />
          ) : (
