...
import { BOOKING_POLICY, CANCELLATION_POLICY, TERMS_AND_CONDITIONS } from "@/data/sitePolicies"
...
function ItineraryAccordion({ itinerary, images, heroImage, typeColor }: {
          ...
          <div key={day.day} className="border border-gray-200 rounded-xl overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                <span className="text-xs font-bold border rounded-full px-3 py-1 bg-white" style={{ color: typeColor, borderColor: typeColor }}>
                  Day {day.day}
                </span>
...
export default function CustomItineraryTabs({ pkg }: { pkg: CustomPackage }) {
      ...
      <div className="grid grid-cols-5 gap-2 mb-6 w-full">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setActive(t)}
            className="flex items-center justify-center gap-1.5 w-full text-sm font-semibold px-2 py-2.5 rounded-xl shadow-sm transition-all border"
            style={
              active === t
                ? { backgroundColor: pkg.typeColor, color: "#fff", borderColor: pkg.typeColor }
      ...
      {active === "Overview" && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h2 className="text-base font-bold text-gray-900 mb-2">About This Itinerary</h2>
            <p className="text-sm text-gray-600 leading-relaxed">
              A handcrafted {pkg.duration} {pkg.type.toLowerCase()} itinerary covering {pkg.cities}, {pkg.country}. Every detail is planned around what actually matters for this kind of trip.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {pkg.highlights.slice(0, 4).map((h) => (
                <div key={h} className="flex flex-col items-center text-center gap-1.5 rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow" style={{ backgroundColor: `${pkg.typeColor}12` }}>
                  <MapPin size={18} style={{ color: pkg.typeColor }} />
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
              {[
                pkg.inclusions.flights ? { icon: "✈️", label: pkg.inclusions.flights } : null,
                { icon: "🚌", label: pkg.inclusions.transfers },
                { icon: "🏨", label: pkg.inclusions.hotel },
                ...
                <div key={f.label} className="flex flex-col items-center justify-center gap-2 border rounded-2xl p-3 shadow-sm hover:shadow-md transition-all" style={{ background: `linear-gradient(to bottom, ${pkg.typeColor}12, #fff)`, borderColor: `${pkg.typeColor}30` }}>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h2 className="text-base font-bold text-gray-900 mb-3">Trip Type</h2>
      ...
      {active === "Itinerary" && (
        <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6">
          ...
          {pkg.itinerary.length > 0 ? (
            <ItineraryAccordion itinerary={pkg.itinerary} images={pkg.images} heroImage={pkg.heroImage} typeColor={pkg.typeColor} />
          ) : (
