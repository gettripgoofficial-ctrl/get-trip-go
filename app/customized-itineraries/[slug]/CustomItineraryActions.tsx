...
export default function CustomItineraryActions({ packageName, priceINR, typeColor }: { packageName: string; priceINR: number; typeColor: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t" style={{ background: `linear-gradient(to right, ${typeColor}10, ${typeColor}18)`, borderColor: `${typeColor}25` }}>
          <span className="text-sm font-bold" style={{ color: typeColor }}>Total <span className="text-[11px] font-normal opacity-70">({adults} adult{adults > 1 ? "s" : ""}{children > 0 ? ` + ${children} child${children > 1 ? "ren" : ""}` : ""})</span></span>
          <span className="text-lg font-extrabold" style={{ color: typeColor }}>{totalPrice}</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <p className="text-xs font-bold text-gray-700 mb-3">Share this itinerary</p>
      ...
      <button onClick={openCustomizeWhatsApp}
        ...
        <div>
          <p className="text-sm font-bold text-green-700">Customize This Itinerary</p>
          <p className="text-[11px] text-green-500 mt-0.5">Modify cities, hotels, meals & more</p>
        </div>
      </button>
